...
export function MarkdownPreview() {
  const [markdown, setMarkdown] = useState<string>("# Markdown预览\n\n这是一个**Markdown**预览工具。\n\n## 功能特点\n\n- 实时预览\n- 可收起的编辑器\n- 支持标准Markdown语法\n\n### 列表示例\n\n1. 第一项\n2. 第二项\n3. 第三项\n\n### 代码示例\n\n```javascript\nfunction helloWorld() {\n  console.log('Hello, World!');\n}\n```\n\n> 这是一个引用块\n\n[链接示例](https://example.com)\n");
  ...
  useEffect(() => {
    ...
    if (shareId) {
      ...
      // 获取分享内容
      fetch(`/api/markdown-share?id=${shareId}`)
        .then(res => {
          if (!res.ok) {
  ...
  const handleShare = async () => {
    ...
    try {
      const response = await fetch('/api/markdown-share', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="w-full max-w-6xl space-y-8">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Markdown编辑器</h2>
      ...
      {showShareSuccess && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
          ...
          <span className="block sm:inline"> 分享链接已生成，有效期10分钟。</span>
        </div>
      )}
      ...
      {shareUrl && (
        <Card className="shadow-md border border-gray-200 dark:border-gray-700">
          <CardHeader className="pb-4">
            <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">分享链接</h3>
          </CardHeader>
          <CardBody className="pt-0">
            <div className="flex items-center space-x-2">
              <div className="flex-1 bg-gray-100 dark:bg-gray-800 p-3 rounded-md overflow-x-auto">
                <code className="text-sm text-gray-800 dark:text-gray-200">{shareUrl}</code>
              </div>
              <Button
                color="primary"
      ...
      {showEditor && (
        <Card className="shadow-md border border-gray-200 dark:border-gray-700">
          <CardHeader className="pb-4">
            <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Markdown输入</h3>
          </CardHeader>
          <CardBody className="pt-0">
            <Textarea
      ...
      <Card className="shadow-md border border-gray-200 dark:border-gray-700">
        <CardHeader className="pb-4">
          <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">预览结果</h3>
        </CardHeader>
        <CardBody className="pt-0">
